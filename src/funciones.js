 ...
 */
export function FiltrarEstados(Tareas, estados, seleccion) {
    const ind = seleccion - 1;
    const estadoSeleccionado = estados[ind] ?? null; //devuelve el de la derecha si el de la izquiera es nullo (coalescencia nula)
 ...
 */
export function VerTodasLasTareas(Tareas) {
    return [...Tareas];
}
//Comprueba si el arreglo contiene elementos. Más conciso y puro.
export function ValidarArreglo(Tareas) {
 ...
 */
export function ImprimirTitulos(tareas) {
 ...
 */
export function buscarTarea(tareas, titulo) {
