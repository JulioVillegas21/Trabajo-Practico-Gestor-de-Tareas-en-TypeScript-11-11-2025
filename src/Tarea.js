...
;
export function mostrarse(Tarea) {
...
export function mostrarTitulo(tarea) {
    return `Titulo: ${tarea.titulo}`;
}
 ...
 */
export function editarTarea(tareas, Estados, Dificultad) {
 ...
 */
export function editarTareaPorIndice(tareas, pos, cambios) {
    if (pos < 0 || pos >= tareas.length)
        return tareas;
    const ahora = new Date();
    const tareaActual = tareas[pos];
    const tareaEditada = { ...tareaActual, ...cambios, fechaultimaEdicion: ahora };
    return [...tareas.slice(0, pos), tareaEditada, ...tareas.slice(pos + 1)];
}
export function recibirDato(Dificultades) {
    ...
    let descripcion = prompt("Ingrese la descripcion de la tarea(opcional): ");
    let dificultad = prompt("Ingrese la dificultad de la tarea ([1]facil,[2] medio,[3] dificil)(opcional): ");
    if (dificultad != "") {
    ...
    let vencimiento1 = prompt("Ingrese la fecha de vencimiento de la tarea (YYYY-MM-DD) o deje en blanco si no tiene: ");
